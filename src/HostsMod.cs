...
namespace HostsFileEditor
{
    ...
    internal class HostsMod
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="HostsMod"/> class.
        /// </summary>
        public HostsMod()
        {
        ...
        /// name cannot be null
        /// </exception>
        public HostsMod(string name)
        {
            name.ThrowIfNull("name");

            this.FilePath = Path.Combine(HostsModList.ModDirectory, name);
        }
        ...
        public static bool Validate(string filePath, out string error)
        {
            ...
            if (isValid)
            {
                if (Directory.Exists(HostsModList.ModDirectory))
                {
                    if (Directory.GetFiles(HostsModList.ModDirectory)
                        .Select(fullFilePath => Path.GetFileName(fullFilePath))
                        .Contains(filePath))
                    {
                        isValid = false;
                        error = Resources.ModExists;
                    }
                }
            }
